//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCLensExplorerDependencyProviderProtocol-Protocol.h"

@class NSString, SCLazy, SCLensExplorerDebugPresenter, SCObservable, SCUserSession;
@protocol SCCache, SCFeatureLensCarousel, SCFeatureLensFeed, SCImageDownloading, SCLensExplorerNavigationProviderProtocol, SCLensExplorerStudySettingsProviderProtocol, SCRequestManager;

@interface SCLensExplorerDependencyProviderImpl : NSObject <SCLensExplorerDependencyProviderProtocol>
{
    SCUserSession *_userSession;
    id <SCLensExplorerNavigationProviderProtocol> _navigationDelegateProvider;
    id <SCFeatureLensCarousel> _lensCarouselFeature;
    id <SCFeatureLensFeed> _lensFeedFeature;
    SCLensExplorerDebugPresenter *_debugPresenter;
    SCLazy *_imageDownloader;
    SCLazy *_requestManager;
    SCLazy *_infoCardPresenter;
    id <SCCache> _animationCache;
    id <SCCache> _stickerCache;
    id <SCCache> _impressionCache;
    id <SCLensExplorerStudySettingsProviderProtocol> _studySettingsProvider;
}

- (void).cxx_destruct;
- (id)infoCardPresenter;
- (id)snapTokenProvider;
- (id)userSession;
@property(readonly, nonatomic) id <SCLensExplorerStudySettingsProviderProtocol> studySettingsProvider; // @synthesize studySettingsProvider=_studySettingsProvider;
@property(readonly, nonatomic) SCObservable *activeLensOrderObservable;
- (id)unlockActionProvider;
- (id)profilePresenter;
- (id)encryptedCacheWithName:(id)arg1 cacheSizePolicy:(id)arg2;
- (id)createDebugViewControllerForStoryId:(id)arg1 debugHtml:(id)arg2 lastStreamToken:(id)arg3;
- (id)userPreferences;
- (id)lensUserProvider;
- (id)blacklistFilter;
- (id)bitmojiImageFetcher;
- (id)liveLensPreviewCameraPresenter;
- (id)searchV1Controller;
- (id)createSearchViewPresenterWithLensExplorerUnlocker:(id)arg1 parentView:(id)arg2 performanceLogger:(id)arg3;
- (id)lensUnlocker;
- (id)createLensExplorerRankingLoggerWithQueuePerformer:(id)arg1;
@property(readonly, nonatomic) id <SCCache> impressionCache; // @synthesize impressionCache=_impressionCache;
@property(readonly, nonatomic) id <SCCache> stickerCache; // @synthesize stickerCache=_stickerCache;
@property(readonly, nonatomic) id <SCCache> animationCache; // @synthesize animationCache=_animationCache;
@property(readonly, nonatomic) id <SCRequestManager> requestManager;
@property(readonly, nonatomic) id <SCImageDownloading> imageDownloader;
- (id)initWithUserSession:(id)arg1;
- (id)initWithUserSession:(id)arg1 navigationDelegateProvider:(id)arg2 lensCarouselFeature:(id)arg3 lensFeedFeature:(id)arg4 infoCardPresenter:(id)arg5;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

