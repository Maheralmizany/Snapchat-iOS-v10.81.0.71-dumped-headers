//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UICollectionViewController.h>

#import "SCPageNameLogging-Protocol.h"
#import "SIGViewController-Protocol.h"

@class NSString, SIGFooterItem, SIGHeaderItem;

@interface SCCommerceCheckoutBaseViewController : UICollectionViewController <SIGViewController, SCPageNameLogging>
{
    SIGHeaderItem *_headerItem;
}

@property(readonly, nonatomic) SIGHeaderItem *headerItem; // @synthesize headerItem=_headerItem;
- (void).cxx_destruct;
- (void)viewDidLoad;
- (id)init;
- (id)attributedPage;
- (id)pageViewName;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(retain, nonatomic) SIGFooterItem *footerItem;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

