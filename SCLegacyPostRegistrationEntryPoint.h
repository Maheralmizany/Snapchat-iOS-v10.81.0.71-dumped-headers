//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SCEntryPoint.h"

#import "SCRegisterV2AddFriendsControllerDelegate-Protocol.h"
#import "SCTraceEnabled-Protocol.h"

@class NSString, SCApplicationStorageServices, SCPostRegistrationScope, SCPostRegistrationWorkflow, SCUserInfoServices, SCUserSessionScope;

@interface SCLegacyPostRegistrationEntryPoint : SCEntryPoint <SCTraceEnabled, SCRegisterV2AddFriendsControllerDelegate>
{
    SCPostRegistrationWorkflow *_workflow;
    SCApplicationStorageServices *_applicationStorageServices;
    SCUserInfoServices *_userInfoServices;
    SCUserSessionScope *_userSessionScope;
    SCPostRegistrationScope *_postRegistrationScope;
}

@property(nonatomic) __weak SCPostRegistrationScope *postRegistrationScope; // @synthesize postRegistrationScope=_postRegistrationScope;
@property(nonatomic) __weak SCUserSessionScope *userSessionScope; // @synthesize userSessionScope=_userSessionScope;
@property(nonatomic) __weak SCUserInfoServices *userInfoServices; // @synthesize userInfoServices=_userInfoServices;
@property(nonatomic) __weak SCApplicationStorageServices *applicationStorageServices; // @synthesize applicationStorageServices=_applicationStorageServices;
- (void).cxx_destruct;
- (void)addFriendsCompleted;
- (id)_generateLegacyAddFriendsViewController;
- (void)begin;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

