//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCLensDataFetcherListener-Protocol.h"
#import "SCLensDataProviderListener-Protocol.h"

@class NSArray, NSMutableDictionary, NSString, SCExperimentManager, SCLensDataFetcher, SCLensInPreviewUsageNotifier, SCSmartCarouselFilterArranger;
@protocol SCLensCameraScreenDataProviderProtocol, SCLensIconInMemoryCache, SCPreviewFeatureSnap3DLensFiltersProviderDelegate, SCPreviewLensDataProviderFactoryProtocol;

@interface SCPreviewFeatureSnap3DLensFiltersProvider : NSObject <SCLensDataProviderListener, SCLensDataFetcherListener>
{
    SCSmartCarouselFilterArranger *_filterArranger;
    SCLensInPreviewUsageNotifier *_lensUsageNotifier;
    id <SCPreviewLensDataProviderFactoryProtocol> _lensDataProviderFactory;
    id <SCLensCameraScreenDataProviderProtocol> _lensDataProvider;
    SCLensDataFetcher *_lensDataFetcher;
    SCExperimentManager *_experimentManager;
    NSString *_lensDataProviderToken;
    NSString *_excludedLensId;
    NSArray *_lenses;
    NSMutableDictionary *_filterConfigs;
    id <SCLensIconInMemoryCache> _lensIconInMemoryCache;
    id <SCPreviewFeatureSnap3DLensFiltersProviderDelegate> _delegate;
}

@property(readonly, nonatomic) __weak id <SCPreviewFeatureSnap3DLensFiltersProviderDelegate> delegate; // @synthesize delegate=_delegate;
- (void).cxx_destruct;
- (void)lensDataFetcher:(id)arg1 didFinishLoadingFilterOverlayImageForLens:(id)arg2 imagePath:(id)arg3 error:(id)arg4;
- (void)lensDataFetcher:(id)arg1 willStartLoadingFilterOverlayImagePathForLens:(id)arg2;
- (void)lensDataFetcher:(id)arg1 didFinishLoadingContentForAsset:(id)arg2 lens:(id)arg3 content:(id)arg4 error:(id)arg5;
- (void)lensDataFetcher:(id)arg1 willStartLoadingAsset:(id)arg2;
- (void)lensDataFetcher:(id)arg1 willStartLoadingImageForLens:(id)arg2;
- (void)lensDataFetcher:(id)arg1 willStartLoadingContentForLens:(id)arg2;
- (void)lensDataFetcher:(id)arg1 didFinishLoadingImageForLens:(id)arg2 image:(id)arg3 error:(id)arg4;
- (void)lensDataFetcher:(id)arg1 didFinishLoadingContentForLens:(id)arg2 contentPath:(id)arg3 error:(id)arg4;
- (void)lensDataProvider:(id)arg1 didUpdateLens:(id)arg2 contentUpdateType:(long long)arg3;
- (void)lensDataProvider:(id)arg1 didUpdateAllLenses:(id)arg2 requiresAnimation:(_Bool)arg3;
- (void)addSwipeFilterForLens:(id)arg1;
- (id)_addSwipeFilterForLens:(id)arg1 normalizedLensIndex:(float)arg2;
- (void)_tryCreateLensFilterWithLens:(id)arg1;
- (void)startUpdatingLensFilterDataExcludingLensId:(id)arg1;
- (id)initWithDelegate:(id)arg1 filterArranger:(id)arg2 lensUsageNotifier:(id)arg3 lensDataProviderFactory:(id)arg4 lensDataFetcher:(id)arg5 experimentManager:(id)arg6 lensIconInMemoryCache:(id)arg7;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

