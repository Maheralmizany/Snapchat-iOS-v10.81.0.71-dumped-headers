//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCUnifiedActionMenuDataProvider-Protocol.h"

@class NSString, SCCheetahPromotedStory, SCDiscoverFeedStory, SCExperimentManager, UIImage;
@protocol SCUnifiedActionMenuDataProviderDelegate;

@interface SCPromotedStoryActionSheetDataProvider : NSObject <SCUnifiedActionMenuDataProvider>
{
    SCDiscoverFeedStory *_story;
    SCCheetahPromotedStory *_promotedStory;
    UIImage *_coverImage;
    SCExperimentManager *_experimentManager;
    id <SCUnifiedActionMenuDataProviderDelegate> _delegate;
}

@property(nonatomic) __weak id <SCUnifiedActionMenuDataProviderDelegate> delegate; // @synthesize delegate=_delegate;
- (void).cxx_destruct;
- (id)_viewModel;
- (void)reload;
- (void)updateViewModelWithCompletionBlock:(CDUnknownBlockType)arg1;
- (void)dealloc;
- (id)initWithStory:(id)arg1 coverImage:(id)arg2 experimentManager:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

