//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCTraceEnabled-Protocol.h"

@class NSString, SCExperimentManager, SCImageProcessLensCommandFactory, SCLens, SCLensDataFetcher, SCLensInMemoryAssetsDataProvider, SCLensSnap3DAssetMetadata;
@protocol SCLensUnlockerProtocol;

@interface SCLensSnap3DLensCommandGenerator : NSObject <SCTraceEnabled>
{
    SCLensDataFetcher *_lensDataFetcher;
    SCExperimentManager *_experimentManager;
    id <SCLensUnlockerProtocol> _lensUnlocker;
    SCLens *_activeLens;
    CDUnknownBlockType _lensCommandGenerationCompletion;
    SCLensSnap3DAssetMetadata *_snap3DAssetMetadata;
    SCLensInMemoryAssetsDataProvider *_inMemoryAssetsProvider;
    SCImageProcessLensCommandFactory *_imageProcessLensCommandFactory;
}

- (void).cxx_destruct;
- (void)_tryActivatingLensWithId:(id)arg1;
- (void)_setBundledLensActive;
- (void)_fetchLens:(id)arg1;
- (id)_createLensCommandWithRemoteAssetsProvider:(id)arg1 uriServiceProvider:(id)arg2 inMemoryAssetsProvider:(id)arg3 snap3DMetadata:(id)arg4;
- (void)_completeLensCommandGeneration:(_Bool)arg1;
- (void)generateSnap3DLensWithId:(id)arg1 snap3DAssetMetadata:(id)arg2 completion:(CDUnknownBlockType)arg3;
- (id)initWithLensDataFetcher:(id)arg1 lensUnlocker:(id)arg2 inMemoryAssetsProvider:(id)arg3 experimentManager:(id)arg4 imageProcessLensCommandFactory:(id)arg5;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

