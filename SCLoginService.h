//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCLoginService-Protocol.h"

@class NSString, SCAppSession, SCFideliusLoggedOutManager, SCLogger, SCSessionRequestManager;
@protocol SCDeviceCheckTokenFetching, SCTransitionMomentLogger;

@interface SCLoginService : NSObject <SCLoginService>
{
    SCSessionRequestManager *_requestManager;
    SCLogger *_logger;
    id <SCTransitionMomentLogger> _transitionMomentLogger;
    id <SCDeviceCheckTokenFetching> _deviceCheckManager;
    SCFideliusLoggedOutManager *_fideliusLoggedOutManager;
    SCAppSession *_appSession;
}

- (void).cxx_destruct;
- (void)warmUserPreLogin:(id)arg1;
- (void)logInWithOdlvChallenge:(id)arg1 solution:(id)arg2 success:(CDUnknownBlockType)arg3 failure:(CDUnknownBlockType)arg4;
- (void)complete2FALogInWithUsernameOrEmail:(id)arg1 confirmationCode:(id)arg2 twoFAPreAuthToken:(id)arg3 rememberDevice:(_Bool)arg4 twoFAMethod:(int)arg5 success:(CDUnknownBlockType)arg6 failure:(CDUnknownBlockType)arg7;
- (void)logInWithUsername:(id)arg1 password:(id)arg2 confirmReactivation:(_Bool)arg3 success:(CDUnknownBlockType)arg4 failure:(CDUnknownBlockType)arg5;
- (id)_makeLoginPayloadWithUsernameOrEmail:(id)arg1 password:(id)arg2 preAuthToken:(id)arg3 twoFAMethod:(int)arg4 odlvPreAuthToken:(id)arg5 otpType:(long long)arg6 confirmReactivation:(_Bool)arg7 rememberDevice:(id)arg8 fideliusClientInit:(id)arg9 deviceCheckToken:(id)arg10;
- (id)_formatLoginParameter:(id)arg1;
- (void)loginWithUsernameOrEmail:(id)arg1 password:(id)arg2 preAuthToken:(id)arg3 twoFAMethod:(int)arg4 odlvPreAuthToken:(id)arg5 otpType:(long long)arg6 confirmReactivation:(_Bool)arg7 rememberDevice:(_Bool)arg8 fideliusClientInit:(id)arg9 successBlock:(CDUnknownBlockType)arg10 failureBlock:(CDUnknownBlockType)arg11;
- (id)initWithRequestManager:(id)arg1 logger:(id)arg2 transitionMomentLogger:(id)arg3 deviceCheckManager:(id)arg4 fideliusLoggedOutManager:(id)arg5 appSession:(id)arg6;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

