//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "LSAEffectComponentListener-Protocol.h"

@class NSString;
@protocol SCLensCrashLoggerProtocol, SCLensPreferences;

@interface SCLensCorePersistentStore : NSObject <LSAEffectComponentListener>
{
    id <SCLensPreferences> _lensPreferences;
    id <SCLensCrashLoggerProtocol> _lensCrashLogger;
}

- (void).cxx_destruct;
- (void)effectComponent:(id)arg1 willTurnOnEffectWithId:(id)arg2;
- (void)effectComponent:(id)arg1 effectId:(id)arg2 savePersistentStore:(id)arg3;
- (void)effectComponent:(id)arg1 loadPersistentStoreForEffectWithId:(id)arg2;
- (id)persistedDataForLensId:(id)arg1;
- (id)initWithPreferences:(id)arg1 lensCrashLogger:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

