//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SCSojuMessage-Protocol.h"

@class NSNumber, NSString;

@protocol SOJUHereAuth <SCSojuMessage>
@property(readonly, nonatomic) NSString *mac;
@property(readonly, nonatomic) NSString *chatToken;
@property(readonly, nonatomic) NSString *signature;
@property(readonly, nonatomic) NSNumber *expires;
@property(readonly, nonatomic) NSString *salt;
@property(readonly, nonatomic) NSNumber *userId;
@property(readonly, nonatomic) NSString *scopeId;
@end

