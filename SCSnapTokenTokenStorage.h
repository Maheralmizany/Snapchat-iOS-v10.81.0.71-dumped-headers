//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCSnapTokenTokenStorageProtocol-Protocol.h"

@class NSMutableDictionary, NSString, SCQueuePerformer;
@protocol SCSnapTokenLogger;

@interface SCSnapTokenTokenStorage : NSObject <SCSnapTokenTokenStorageProtocol>
{
    NSMutableDictionary *_accessTokens;
    id <SCSnapTokenLogger> _logger;
    _Bool _invalidated;
    NSString *_refreshToken;
    SCQueuePerformer *_performer;
    SCQueuePerformer *_keychainPerformer;
}

+ (id)fetchOperationFromAccessType:(unsigned long long)arg1 queue:(id)arg2;
+ (id)accessTokenKeychainEntryForUserId:(id)arg1 accessType:(unsigned long long)arg2;
+ (id)refreshTokenKeychainEntryForUserId:(id)arg1;
@property(readonly, nonatomic) SCQueuePerformer *keychainPerformer; // @synthesize keychainPerformer=_keychainPerformer;
@property(readonly, nonatomic) SCQueuePerformer *performer; // @synthesize performer=_performer;
@property(nonatomic) _Bool invalidated; // @synthesize invalidated=_invalidated;
@property(retain, nonatomic) NSString *refreshToken; // @synthesize refreshToken=_refreshToken;
- (void).cxx_destruct;
- (void)_writeKeychainRefreshTokenBackground:(id)arg1 userId:(id)arg2;
- (void)_writeKeychainAccessTokenBackground:(id)arg1 accessType:(unsigned long long)arg2 userId:(id)arg3;
- (id)_readRefreshTokenKeychainWithUserId:(id)arg1;
- (void)_clearAccessTokenKeychainSyncWithAccessType:(unsigned long long)arg1 userId:(id)arg2;
- (id)_readAccessTokenKeychainWithAccessType:(unsigned long long)arg1 userId:(id)arg2;
- (void)_setAccessTokenValue:(id)arg1 forAccessTypeKey:(id)arg2;
- (void)_removeAccessTokenForAccessTypeKey:(id)arg1;
- (id)_accessTokenForAccessTypeKey:(id)arg1;
- (_Bool)_isValidAccessToken:(id)arg1 accessType:(unsigned long long)arg2;
- (id)_getInMemoryAccessTokenForOp:(id)arg1;
- (void)_handleInvalidationForUserId:(id)arg1;
- (void)_loadRefreshTokenFromKeychainForUserId:(id)arg1;
- (void)_loadAccessTokenFromKeyChainForUserId:(id)arg1 accessType:(unsigned long long)arg2;
- (void)_writeMemAndKeychainAccessToken:(id)arg1 accessType:(unsigned long long)arg2 userId:(id)arg3;
- (void)_writeMemAndKeychainAccessTokens:(struct NSDictionary *)arg1 userId:(id)arg2;
- (void)_clearMemAndKeychainAccessTokenWithUserId:(id)arg1 token:(id)arg2 accessType:(unsigned long long)arg3;
- (void)_writeMemAndKeychainRefreshToken:(id)arg1 userId:(id)arg2;
- (void)_writeMemAndKeychainWithRefreshToken:(id)arg1 accessTokens:(struct NSDictionary *)arg2 userId:(id)arg3;
- (id)_getRefreshTokenForUserId:(id)arg1;
- (id)_getAccessTokenForUserId:(id)arg1 op:(id)arg2;
- (void)clearAllAccessTokensSyncWithUserId:(id)arg1;
- (void)handleInvalidationSyncForUserId:(id)arg1;
- (void)clearInvalidAccessTokenAsyncWithUserId:(id)arg1 token:(id)arg2 accessType:(unsigned long long)arg3;
- (void)setAccessTokensSyncWithUserId:(id)arg1 accessTokens:(struct NSDictionary *)arg2;
- (id)getAccessTokenSyncForAccessType:(unsigned long long)arg1 userId:(id)arg2;
- (void)getAccessTokenAsyncForFetchOperation:(id)arg1 userId:(id)arg2 completionPerformer:(id)arg3 completion:(CDUnknownBlockType)arg4;
- (void)setSnapSessionSyncWithRefreshToken:(id)arg1 accessTokens:(struct NSDictionary *)arg2 userId:(id)arg3;
- (void)getRefreshTokenAsyncWithCompletionPerformer:(id)arg1 userId:(id)arg2 completion:(CDUnknownBlockType)arg3;
- (id)initWithLogger:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

